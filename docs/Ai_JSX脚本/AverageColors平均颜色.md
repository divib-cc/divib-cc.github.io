...
对所选对象的颜色进行平均，或在组或梯度内分别进行平均。脚本会跳过没有颜色或有图案的对象。如果没有选择任何东西，脚本会处理文档中的所有组。它有两种模式：无声和对话。在`CFG.showUI`中改变。  

如果你在运行脚本时按住 <kbd>Alt</kbd> 键，模式的改变不需要编辑代码:

* <kbd>Alt</kbd> + `CFG.showUI: false` 将会显示对话框。
* <kbd>Alt</kbd> + `CFG.showUI: true` 无声模式，有最新的选项

![AverageColors](./AverageColors.gif) 
